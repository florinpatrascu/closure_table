defmodule CTE.Registry do
  ...
  def lookup(cte) when is_atom(cte) do
    GenServer.whereis(cte)
    |> Kernel.||(
      raise "could not lookup #{inspect(cte)} because it was not started or it does not exist"
    )
    |> lookup()
  end
